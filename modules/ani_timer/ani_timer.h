...
class AniTimer : public Timer
{
	...
	public:
		...
		void prepareAniCustomFrame(Object* p_sprite, int customStartFrame);
		void stTime();
		void timerDone();
		int getLastFrame();
		...
		int getFrameNumber();
		void setStartFrame(int customStartFrame);
		int getStartFrame();
		void setStateStartFrame(const Dictionary& customStateStartFrame);
