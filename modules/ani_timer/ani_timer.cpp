...
{
	set_one_shot(true);
	connect("timeout", callable_mp(this, &AniTimer::timerDone));
	frameReady = true;
	frameNum = 0;
	lastFrame = 0;
	startFrame = 0;
...
void AniTimer::prepareAniCustomFrame(Object* p_sprite, int customStartFrame)
{
	Sprite3D* sprite = Object::cast_to<Sprite3D>(p_sprite);
	lastFrame = frameNum;
	sprite->set_frame(customStartFrame);
	frameNum = customStartFrame;
}
...
void AniTimer::setStartFrame(int customStartFrame)
{
	this->startFrame = customStartFrame;
}
...
void AniTimer::setStateStartFrame(const Dictionary& customStateStartFrame)
{
