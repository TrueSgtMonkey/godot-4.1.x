...
*/
int SpriteRotater::rotationOffset(float rot, int angleFrame)
{
	if (rot > 337.5 && rot <= 360.0 || rot > 0.0 && rot <= 22.5)
		return angleFrame;
	else if (rot > 22.5 && rot <= 67.5)
		angleFrame = frameChanger(angleFrame, 1, 0, 8);
	else if (rot > 67.5 && rot <= 112.5)
		angleFrame = frameChanger(angleFrame, 2, 1, 8);
	else if (rot > 112.5 && rot <= 157.5)
		angleFrame = frameChanger(angleFrame, 3, 2, 8);
	else if(rot > 157.5 && rot <= 202.5)
		angleFrame = frameChanger(angleFrame, 4, 3, 8);
	else if(rot > 202.5 && rot <= 247.5)
		angleFrame = frameChanger(angleFrame, 5, 4, 8);
	else if(rot > 247.5 && rot <= 292.5)
		angleFrame = frameChanger(angleFrame, 6, 5, 8);
	else if(rot > 292.5 && rot <= 337.5)
		angleFrame = frameChanger(angleFrame, 7, 6, 8);
	return angleFrame;
}
 ...
 */
int SpriteRotater::frameChanger(int currFrame, int subtractor, int minFrame, int maxFrame)
{
	if(currFrame > minFrame)
		currFrame -= subtractor;
	else
		currFrame = maxFrame - (subtractor - currFrame);
	return currFrame;
}

int SpriteRotater::adjustFrame(Vector3 dir, int currFrame, float rotation, int subtractor)
{
	// getting the frame based on direction moving
	currFrame = rotater(dir);
	
	// offsetting the frame based on the camera rotation
	currFrame = rotationOffset(rotation, currFrame);
	currFrame = frameChanger(currFrame, subtractor, subtractor - 1, 8);
	return currFrame;
}
...
int SpriteRotater::rotater(Vector3 vec)
{
	...
	if(vec.z != 0)
		ang = Math::rad_to_deg(atan(vec.x / vec.z));
	frame = 0;
	if (vec.x < 0)
	{
...
Vector2 SpriteRotater::get2DFrameVector(int angleFrame)
{
	Vector2 vec;
	switch(angleFrame)
	{
...
Vector3 SpriteRotater::get3DFrameVector(int currFrame)
{
	Vector3 vec;
	vec.y = 0;
	switch(currFrame)
...
Vector3 SpriteRotater::idleTurn(Object* p_sprite, int totalFrame, int row, int curr_frame)
{
	Sprite3D* sprite = Object::cast_to<Sprite3D>(p_sprite);
	sprite->set_frame(totalFrame * row);
	sprite->set_frame(sprite->get_frame() + (curr_frame % row));
	return get3DFrameVector(totalFrame);
}
