...
class SpriteRotater : public RefCounted
{
	GDCLASS(SpriteRotater, RefCounted);
	...
	public:
		int rotationOffset(float rot, int angleFrame);
		int frameChanger(int currFrame, int subtractor, int minFrame, int maxFrame);
		int adjustFrame(Vector3 dir, int angleFrame, float rotation, int subtractor = 2);
		float getRotation(float rotation);
		int rotater(Vector3 vec);
		int get2DVectorFrame(Vector2 vec);
		int get3DVectorFrame(Vector3 vec);
		Vector2 get2DFrameVector(int angleFrame);
		Vector3 get3DFrameVector(int angleFrame);
		...
		Vector3 idleTurn(Object* p_sprite, int angleFrame, int row, int curr_frame);
};
